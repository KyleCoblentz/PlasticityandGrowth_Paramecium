...
tpc_fit <- gam(formula = Growth.Rate.Hours ~ Line + s(Temperature, k = 5, bs = 'tp', by = Line), data = growth_data, method = 'REML')

summary(tpc_fit)

tpc_fit_noint <- gam(formula = Growth.Rate.Hours ~ Line + s(Temperature, k = 5, bs = 'tp'), data = growth_data, method = 'REML')
...
tpc_means <- growth_data %>% filter(Temperature <= 34) %>% group_by(Line, Temperature) %>% 
...
ggplot(data = plast_growth_data, aes(x = range_length, y = range_growth)) +
